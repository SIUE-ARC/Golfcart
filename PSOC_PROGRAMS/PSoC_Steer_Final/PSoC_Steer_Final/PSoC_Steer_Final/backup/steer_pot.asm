...
;  Global Symbols
;-----------------------------------------------
export  Steer_Pot_Start
export _Steer_Pot_Start
export  Steer_Pot_SetPower
export _Steer_Pot_SetPower

export  Steer_Pot_SetGain
export _Steer_Pot_SetGain

export  Steer_Pot_Stop
export _Steer_Pot_Stop

IF (Steer_Pot_AGNDBUFAPI)
export  Steer_Pot_EnableAGNDBuffer
export _Steer_Pot_EnableAGNDBuffer
export  Steer_Pot_DisableAGNDBuffer
export _Steer_Pot_DisableAGNDBuffer
ENDIF
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Steer_Pot_Start
;  FUNCTION NAME: Steer_Pot_SetPower
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[Steer_Pot_GAIN_CR2]                         ; read power value
   and  A, ~POWERMASK                                ; clear power bits in A
   or   A, [X]                                       ; combine power value with balance of reg.
   mov  reg[Steer_Pot_GAIN_CR2], A                    ; move complete value back to register
   pop  A
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Steer_Pot_SetGain
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[Steer_Pot_GAIN_CR0]                         ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[Steer_Pot_GAIN_CR3],~EXGAIN                    ; Clear High Gain bit.
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[Steer_Pot_GAIN_CR0], A                         ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[Steer_Pot_GAIN_CR0], A                         ; move complete value back to register
   or   reg[Steer_Pot_GAIN_CR3], EXGAIN                    ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
...
_Steer_Pot_Stop:
   ...
   and REG[Steer_Pot_GAIN_CR2], ~POWERMASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Steer_Pot_EnableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 Steer_Pot_EnableAGNDBuffer:
_Steer_Pot_EnableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   reg[Steer_Pot_GAIN_CR3], ~AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Steer_Pot_DisableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 Steer_Pot_DisableAGNDBuffer:
_Steer_Pot_DisableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   or    reg[Steer_Pot_GAIN_CR3], AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
ENDIF

; End of File Steer_Pot.asm
