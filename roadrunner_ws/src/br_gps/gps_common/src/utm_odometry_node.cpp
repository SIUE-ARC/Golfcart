...
void Camera(const cmvision::Blobs& blobby);
	...
	//////////////////
		...
		if(((GridLat != GPSGrid[GPSCounter].x-1)|| (GridLat != GPSGrid[GPSCounter].x+1) || (GridLat != GPSGrid[GPSCounter].x)) && ((GridLong != GPSGrid[GPSCounter].y-1) || (GridLong != GPSGrid[GPSCounter].y+1) || (GridLong != GPSGrid[GPSCounter].y)))
		{
			...
			if(i % 5 == 0)
			{
				...
				{
					slope_one = -5000;
					cout << "Slope_One Is Undefined" << std::endl;
				}
				else
				...
				{
					slope_two = -50;
					cout <<"Slope_Two is Undefined" << std::endl;
				}
				else
				...
				//if(slope_one == 0)
				//{
				///*Case: 1*/ if(GPSGrid[GPSCounter].y > GridLong && GPSGrid[GPSCounter].x > GridLat) //Turn Left
			//			turnangle = abs(15)*-1;
		//			    else if(GPSGrid[GPSCounter].y < GridLong && GPSGrid[GPSCounter].x > GridLat) //Turn Right
	//					turnangle = abs(15);
	//			/*Case: 2*/ else if(GPSGrid[GPSCounter].y < GridLong && GPSGrid[GPSCounter].x < GridLat) //Turn Left
	//					turnangle = abs(15)*-1;
	//				    else if(GPSGrid[GPSCounter].y > GridLong && GPSGrid[GPSCounter].x < GridLat) //Turn Right
	//					turnangle = abs(15);
	//			}
				if(slope_one == -5000) //My_Slope = Undefined
				{
				/*Case: 1*/ if(GPSGrid[GPSCounter].y > GridLong && GPSGrid[GPSCounter].x > GridLat) //Turn Right
						turnangle = abs(15);
					    else if(GPSGrid[GPSCounter].y > GridLong && GPSGrid[GPSCounter].x < GridLat) //Turn Left
						turnangle = abs(15)*-1;
				/*Case: 2*/ else if(GPSGrid[GPSCounter].y < GridLong && GPSGrid[GPSCounter].x > GridLat) //Turn Left
						turnangle = abs(15)*-1;
					    else if(GPSGrid[GPSCounter].y < GridLong && GPSGrid[GPSCounter].x < GridLat) //Turn Right
				...
				//Decide what direction we need to turn for non special cases
				//Newy > CurrY && Newx > CurrX && My_Slope < Desired_Slope
	  /*case: 1*/		else if(GPSGrid[GPSCounter].y > GridLong && GPSGrid[GPSCounter].x > GridLat && slope_one < slope_two) //Turn Left
	/* Y > & X > */			turnangle = abs(turnangle)*-1;
				//Newy > CurrY && Newx > CurrX && My_Slope > Desired_Slope
				else if(GPSGrid[GPSCounter].y > GridLong && GPSGrid[GPSCounter].x > GridLat && slope_one > slope_two) //Turn Right
					turnangle = abs(turnangle);
				//Newy < CurrY && Newx > CurrX && My_Slope < Desired_Slope
	  /*case: 2*/		else if(GPSGrid[GPSCounter].y < GridLong && GPSGrid[GPSCounter].x > GridLat && slope_one < slope_two) //Turn Left
	/* Y < & X > */			turnangle = abs(turnangle)*-1;
				//Newy < CurrY && Newx > CurrX && My_Slope > Desired_Slope
				else if(GPSGrid[GPSCounter].y < GridLong && GPSGrid[GPSCounter].x > GridLat && slope_one > slope_two) //Turn Right
					turnangle = abs(turnangle);
				//Newy > CurrY && Newx < CurrX && My_Slope < Desired_Slope
	  /*case: 3*/		else if(GPSGrid[GPSCounter].y > GridLong && GPSGrid[GPSCounter].x < GridLat && slope_one < slope_two) //Turn Right
	/* Y > & X < */			turnangle = abs(turnangle);
				//Newy > CurrY && Newx < CurrX && My_Slope > Desired_Slope
				else if(GPSGrid[GPSCounter].y > GridLong && GPSGrid[GPSCounter].x < GridLat && slope_one > slope_two) //Turn Left
					turnangle = abs(turnangle)*-1;
				//Newy < CurrY && Newx < CurrX && My_Slope > Desired_Slope
	  /*case: 4*/		else if(GPSGrid[GPSCounter].y < GridLong && GPSGrid[GPSCounter].x < GridLat && slope_one > slope_two) //Turn Right
	/* Y < & X < */			turnangle = abs(turnangle);
				//Newy < CurrY && Newx < CurrX && My_Slope < Desired_Slope
				else if(GPSGrid[GPSCounter].y < GridLong && GPSGrid[GPSCounter].x < GridLat && slope_one < slope_two) //Turn Left
					turnangle = abs(turnangle)*-1;
...
/*	if(blobby.blobs.size() > 0)
	{
		cout << "mmmm blob x pos: " << blobby.blobs[0].x << endl;
		cout << "num of blobs:" << blobby.blob_count << endl;
		cout << "size: " << blobby.blobs.size() << endl;
		cout << "area: " << blobby.blobs[0].area << endl;

		if(blobby.blobs[0].x > 320)
		{
			//blob on the right
